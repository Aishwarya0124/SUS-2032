...
/* Bug COR-871, 879*/
    function cartPopulate(data, shippingData){
        ...
        for(var i = 0; i < data.length; i ++){
			...
			var maxQty = itemData["max_OrderQuantity"];
			
			 if (itemData["quantity"] == 2){
             var infoQuantityMarkup1 = '<div class="incrementor"><a class="minus '+productCode+'" href="#" data-direction="minus" data-product="'+productCode+'" data-itemid="' + entryNumber + '" data-entryNumber="'+entryNumber+'">-</a><p class="q-val q-val-parent">' + itemData["quantity"] + '</p><a class="plus plus-parentt" href="#" data-direction="plus" data-product="'+productCode+'" data-itemid="' + entryNumber + '" data-entryNumber="'+entryNumber+'">+</a></div>';
             $("#header-cart .cart-body .cart-list #cart-item-" + entryNumber + " .product-info .quantity-container").append(infoQuantityMarkup1);    
             $('.plus.plus-parentt').addClass('plus-icon-incrementor')
